...
function fakeMessage(overrides: Partial<Message> = {}): Message {
    return {
        ...
        channelId: 'c-1',
        guildId: null,
        author: { id: '111111111111111111', username: 'a', globalName: null, avatarUrl: '', bot: false },
        content: 'hi',
...
describe('SqliteDmInbox', () => {
    ...
    it('upsertChannel rewrites recipient fields on second call (same channel)', async () => {
        ...
        const updated = await inbox.upsertChannel('c-1', { ...RECIPIENT, username: 'alice-new' });
    ...
    it('recordActivity advances lastMessageAt + lastMessageId on a newer message', async () => {
        const inbox = new SqliteDmInbox();
        await inbox.recordActivity('c-1', RECIPIENT, fakeMessage({
            id: '600000000000000001',
            createdAt: '2026-04-25T12:00:00.000Z',
            content: 'older'
        }));
        const after = await inbox.recordActivity('c-1', RECIPIENT, fakeMessage({
    ...
    it('listChannels orders by lastMessageAt DESC', async () => {
        const inbox = new SqliteDmInbox();
        await inbox.recordActivity('c-old', RECIPIENT, fakeMessage({
            id: '600000000000000010',
            createdAt: '2026-04-20T00:00:00.000Z'
        }));
        await inbox.recordActivity('c-new', RECIPIENT_2, fakeMessage({
    ...
    describe('recipientId unique constraint', () => {
        it('enforces unique recipientId across rows', async () => {
            ...
            await expect(
                inbox.upsertChannel('c-2', RECIPIENT)
            ).rejects.toThrow();
        });
