...
describe('resolveUserCapabilities', () => {
    it('bypasses for the owner — every known capability granted, no DB hit needed', async () => {
        const caps = await resolveUserCapabilities(OWNER_ID, OWNER_ID);
        // Should at minimum include `admin` and the granular tokens.
        expect(caps.has('admin' as AdminCapability)).toBe(true);
        expect(caps.has('dm.read' as AdminCapability)).toBe(true);
        expect(caps.has('guild.read' as AdminCapability)).toBe(true);
    });
    ...
    it('caches results — second call within TTL does not re-hit the DB', async () => {
        ...
        await AdminRoleCapability.create({ role: 'mod', capability: 'guild.read' });
        ...
        expect(caps.has('guild.read' as AdminCapability)).toBe(false);
    });
    ...
    it('global invalidate clears every entry', async () => {
        ...
        await grantRoleCapability('a', 'dm.read');
        ...
        await AdminRoleCapability.create({ role: 'a', capability: 'guild.read' });
        invalidateCapabilityCache();
        ...
        expect(a.has('guild.read' as AdminCapability)).toBe(true);
        expect(b.has('guild.read' as AdminCapability)).toBe(true);
    });
});
...
describe('role CRUD', () => {
    ...
    it('deleteAdminRole removes the role AND its capability rows', async () => {
        ...
        await grantRoleCapability('victim', 'dm.read');
        await grantRoleCapability('victim', 'guild.write');
    ...
    it('grantRoleCapability is idempotent — re-granting the same cap is a no-op', async () => {
        ...
        await grantRoleCapability('a', 'dm.read');
        await grantRoleCapability('a', 'dm.read');
        const grants = await AdminRoleCapability.findAll({ where: { role: 'a', capability: 'dm.read' } });
        expect(grants).toHaveLength(1);
    });
    ...
    it('revokeRoleCapability invalidates every cached user', async () => {
        ...
        expect(before.has('guild.read' as AdminCapability)).toBe(true);
        await revokeRoleCapability('a', 'guild.read');
        const after = await resolveUserCapabilities('111111111111111111', OWNER_ID);
        expect(after.has('guild.read' as AdminCapability)).toBe(false);
    });
});
