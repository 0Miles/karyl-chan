...
interface AdminLike { userId: string; role: string }

const RealisticComponent = defineComponent({
    template: '<div><span v-for="a in admins" :key="a.userId" class="name">{{ display(a.userId) }}</span></div>',
    props: {
...
describe('useUserSummaries reactivity (Pinia store)', () => {
    ...
    it('updates display name after props.admins changes from empty to populated', async () => {
        mockFetch.mockResolvedValue({
            ...
            id2: { id: 'id2', username: 'bob', globalName: null, avatarUrl: '', bot: false },
        });
        ...
        await wrapper.setProps({ admins: [
            { userId: 'id1', role: 'admin' },
            { userId: 'id2', role: 'admin' },
        ] });

        expect(wrapper.findAll('.name').map(el => el.text())).toEqual(['[id1]', '[id2]']);
        expect(mockFetch).toHaveBeenCalledWith(['id1', 'id2']);

        await flushPromises();
        await nextTick();

        expect(wrapper.findAll('.name').map(el => el.text())).toEqual(['Alice', 'bob']);
    });
});
