...
function getCached(
  userId: string,
  now = Date.now(),
): DiscordUserSummary | null | undefined {
...
export function useUserSummaries(
  ...
  async function fetchMissing(ids: string[]) {
    const now = Date.now();
    // Collect cache hits into a separate map so we can trigger reactivity
    // with a single ref reassignment rather than in-place Map mutation
    // (ref(Map) does not track .set() calls — only ref reassignment does).
    const cacheHits = new Map<string, DiscordUserSummary | null>();
    const missing = ids.filter((id) => {
      const cached = getCached(id, now);
      if (cached !== undefined) {
        cacheHits.set(id, cached);
        return false;
      }
    ...
    if (cacheHits.size > 0) {
      // Apply cache hits via ref reassignment to trigger reactive updates.
      const next = new Map(summaries.value);
      for (const [id, val] of cacheHits) next.set(id, val);
