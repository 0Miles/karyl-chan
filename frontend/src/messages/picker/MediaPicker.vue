    ...
    <div class="picker">
        ...
        <div class="body">
            ...
            <template v-else-if="activeTab === 'unicode'">
                <section v-for="cat in unicodeCategories" :key="cat.id" class="section">
                    <h4 class="capitalize">{{ cat.id }}</h4>
                    <div class="grid emoji-grid">
                        <button
            ...
            <template v-else-if="activeTab === 'custom'">
                <section v-for="bucket in customGuilds" :key="bucket.guildId" class="section">
                    <h4>{{ bucket.guildName }}</h4>
                    <div class="grid emoji-grid">
                        <button
                            v-for="emoji in bucket.items"
                            :key="emoji.id"
                            type="button"
                            class="cell"
                            :title="`:${emoji.name}:`"
                ...
                <p v-if="!customGuilds.length" class="muted">No custom emoji available.</p>
            </template>

            <template v-else-if="activeTab === 'sticker'">
                <section v-for="bucket in stickerGuilds" :key="bucket.guildId" class="section">
                    <h4>{{ bucket.guildName }}</h4>
                    <div class="grid sticker-grid">
                        <button
                            v-for="s in bucket.items"
                            :key="s.id"
                            type="button"
                            class="cell sticker-cell"
                            :title="s.name"
                ...
                <p v-if="!stickerGuilds.length" class="muted">No guild stickers available.</p>
            </template>
        </div>
    </div>
</template>
