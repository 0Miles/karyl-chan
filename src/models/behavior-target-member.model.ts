 ...
 */
export const BehaviorTargetMember = sequelize.define('BehaviorTargetMember', {
    targetId: {
...
export const findGroupTargetIdsForUser = async (userId: string): Promise<number[]> => {
    const rows = await BehaviorTargetMember.findAll({ where: { userId } });
    return rows.map(r => r.getDataValue('targetId') as number);
};

export const addGroupMember = async (targetId: number, userId: string): Promise<void> => {
    await BehaviorTargetMember.upsert({ targetId, userId });
};

export const removeGroupMember = async (targetId: number, userId: string): Promise<void> => {
    await BehaviorTargetMember.destroy({ where: { targetId, userId } });
};

export const replaceGroupMembers = async (targetId: number, userIds: string[]): Promise<void> => {
