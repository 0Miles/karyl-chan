 ...
 */
export const Behavior = sequelize.define('Behavior', {
    id: {
...
export const findBehaviorById = async (id: number): Promise<BehaviorRow | null> => {
...
export const createBehavior = async (input: NewBehaviorInput): Promise<BehaviorRow> => {
    ...
    const nextSort = ((maxRow?.getDataValue('sortOrder') as number | undefined) ?? -1) + 1;
...
export const updateBehavior = async (id: number, patch: BehaviorUpdate): Promise<BehaviorRow | null> => {
 ...
 */
export const reorderBehaviors = async (targetId: number, orderedIds: number[]): Promise<void> => {
