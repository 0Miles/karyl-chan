...
async function rebuildAuthorizedUsers(
  sequelize: Sequelize,
...
async function rebuildRoleCapabilities(
  sequelize: Sequelize,
  transaction: import("sequelize").Transaction,
): Promise<void> {
  await sequelize.query(
  ...
  await sequelize.query(
    "ALTER TABLE admin_role_capabilities_new RENAME TO admin_role_capabilities;",
    { transaction },
  );
}
    ...
    try {
      await sequelize.transaction(async (transaction) => {
        if (needsAuthorizedUsers)
          await rebuildAuthorizedUsers(sequelize, transaction);
