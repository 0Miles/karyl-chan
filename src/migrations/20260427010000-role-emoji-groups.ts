...
interface InsertedGroupRow { id: number }
    ...
    up: async ({ queryInterface, sequelize }) => {
        ...
        for (const guildId of guildIds) {
            ...
            const [existing] = await sequelize.query<InsertedGroupRow>(
                ...
                { type: QueryTypes.SELECT, replacements: { guildId, name: DEFAULT_GROUP_NAME } }
            );
            if (existing) {
            ...
            if (!created) throw new Error(`failed to create default group for guild ${guildId}`);
        ...
        try {
            await sequelize.transaction(async (transaction) => {
                ...
                await sequelize.query(`DROP TABLE ${ROLE_EMOJIS_TABLE};`, { transaction });
                await sequelize.query(
