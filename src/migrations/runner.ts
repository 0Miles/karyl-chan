...
export interface Migration {
    up(context: { queryInterface: QueryInterface; sequelize: Sequelize }): Promise<void>;
    down(context: { queryInterface: QueryInterface; sequelize: Sequelize }): Promise<void>;
}

export function createMigrator(): Umzug<{ queryInterface: QueryInterface; sequelize: Sequelize }> {
    return new Umzug({
        migrations: {
            ...
            glob: [resolve(__dirname, '*.{ts,js}'), { ignore: [resolve(__dirname, 'runner.*')] }],
            resolve: ({ name, path: migrationPath, context }) => {
                if (!migrationPath) throw new Error(`migration ${name} has no path`);
                return {
                    name,
                    up: async () => {
                        const mod = (await import(migrationPath)) as { default?: Migration } & Partial<Migration>;
