...
function parseKeys(raw: string): EncKey[] {
    ...
    return hexes.map(hex => {
        ...
        if (bytes.length !== KEY_BYTES) {
            throw new Error(`encryption key must be ${KEY_BYTES} bytes (${KEY_BYTES * 2} hex chars)`);
        }
...
function decryptWith(keyBytes: Buffer, ivB64: string, tagB64: string, ctB64: string): string {
...
export function decryptSecret(value: string): string {
    ...
    if (value.startsWith(`${ENC_VERSION_V2}:`)) {
        ...
        if (!key) throw new Error(`unknown encryption key id ${keyId} — was the key rotated out?`);
    ...
    if (value.startsWith(`${ENC_VERSION_V1}:`)) {
        ...
        for (const key of keys) {
            try {
                return decryptWith(key.bytes, ivB64, tagB64, ctB64);
            } catch { /* GCM auth failure → try next key */ }
        }
