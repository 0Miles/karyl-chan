...
export async function registerBehaviorRoutes(
  ...
  server.post(
    ...
    async (request, reply) => {
      ...
      if (!["custom", "plugin"].includes(body.source)) {
        return reply.code(400).send({ error: "只能建立 custom 或 plugin source 的 behavior" });
      }
      if (!body.triggerType || !["slash_command", "message_pattern"].includes(body.triggerType)) {
      ...
      if (body.source === "plugin") {
        ...
        if (!body.pluginBehaviorKey?.trim()) {
          return reply.code(400).send({ error: "source=plugin 需要 pluginBehaviorKey" });
        }
      }

      // triggerType 相關驗證
      if (body.triggerType === "message_pattern") {
        if (!body.messagePatternKind || !["startswith", "endswith", "regex"].includes(body.messagePatternKind)) {
        ...
        if (body.messagePatternKind === "regex" && !isValidRegex(body.messagePatternValue)) {
      ...
      // 三軸排序
      const integrationTypes = sortedComma(body.integrationTypes || "guild_install");
      ...
      const row = await Behavior.create({
        ...
        messagePatternKind: body.triggerType === "message_pattern" ? body.messagePatternKind : null,
        messagePatternValue: body.triggerType === "message_pattern" ? body.messagePatternValue : null,
        slashCommandName: body.triggerType === "slash_command" ? body.slashCommandName?.trim() : null,
        slashCommandDescription: body.triggerType === "slash_command" ? (body.slashCommandDescription ?? "") : null,
        scope: body.scope ?? "global",
        integrationTypes,
        contexts,
        audienceKind: body.audienceKind ?? "all",
        audienceUserId: body.audienceKind === "user" ? (body.audienceUserId ?? null) : null,
        audienceGroupName: body.audienceKind === "group" ? (body.audienceGroupName ?? null) : null,
        webhookUrl: body.source === "custom" && body.webhookUrl
          ? encryptSecret(body.webhookUrl.trim())
          : null,
        webhookSecret: body.webhookSecret
          ? encryptSecret(body.webhookSecret)
          : null,
        webhookAuthMode: body.webhookSecret ? (body.webhookAuthMode ?? "token") : null,
        pluginId: body.source === "plugin" ? (body.pluginId ?? null) : null,
        pluginBehaviorKey: body.source === "plugin" ? (body.pluginBehaviorKey ?? null) : null,
        systemKey: null,
      ...
      botEventLog.record("info", "web", `behavior 已建立 id=${created.id} source=${created.source}`, {
  ...
  server.patch(
    ...
    async (request, reply) => {
      ...
      if (existingRow.source === "system") {
        ...
        if ("slashCommandName" in body && existingRow.triggerType === "slash_command") {
          const val = (body["slashCommandName"] as string | null)?.trim();
          if (val !== undefined) patch["slashCommandName"] = val;
        }
        if ("messagePatternValue" in body && existingRow.triggerType === "message_pattern") {
          const val = (body["messagePatternValue"] as string | null)?.trim();
          if (val !== undefined) {
            if (existingRow.messagePatternKind === "regex" && val && !isValidRegex(val)) {
              return reply.code(400).send({ error: "regex 格式錯誤" });
            }
        ...
        if ("integrationTypes" in body) {
          patch["integrationTypes"] = sortedComma(body["integrationTypes"] as string);
        }
        if ("contexts" in body) {
          patch["contexts"] = sortedComma(body["contexts"] as string);
        }
        if ("audienceKind" in body) patch["audienceKind"] = body["audienceKind"];
        if ("audienceUserId" in body) patch["audienceUserId"] = body["audienceUserId"] ?? null;
        if ("audienceGroupName" in body) patch["audienceGroupName"] = body["audienceGroupName"] ?? null;
        ...
        if ("webhookSecret" in body) {
          ...
          if (secret === null || secret === "") {
            ...
            patch["webhookAuthMode"] = (body["webhookAuthMode"] as BehaviorWebhookAuthMode) ?? "token";
          }
        ...
        if ("triggerType" in body) {
          if (!["slash_command", "message_pattern"].includes(body["triggerType"] as string)) {
            return reply.code(400).send({ error: "無效的 triggerType" });
          }
          patch["triggerType"] = body["triggerType"];
        }
        if ("messagePatternKind" in body) patch["messagePatternKind"] = body["messagePatternKind"] ?? null;
        if ("messagePatternValue" in body) {
          const val = (body["messagePatternValue"] as string | null)?.trim() ?? null;
          if (val && (body["messagePatternKind"] ?? existingRow.messagePatternKind) === "regex" && !isValidRegex(val)) {
            return reply.code(400).send({ error: "regex 格式錯誤" });
          }
          patch["messagePatternValue"] = val;
        }
        if ("slashCommandName" in body) patch["slashCommandName"] = (body["slashCommandName"] as string | null)?.trim() ?? null;
        if ("slashCommandDescription" in body) patch["slashCommandDescription"] = body["slashCommandDescription"] ?? null;
        if ("scope" in body) patch["scope"] = body["scope"];
        if ("integrationTypes" in body) {
          patch["integrationTypes"] = sortedComma(body["integrationTypes"] as string);
        }
        ...
        if ("webhookSecret" in body) {
          ...
          if (mode && !currentSecret) {
            return reply.code(400).send({ error: "設定 webhookAuthMode 需要先設定 webhookSecret" });
          }
          patch["webhookAuthMode"] = mode ?? null;
        }
        if ("pluginId" in body) patch["pluginId"] = body["pluginId"] ?? null;
        if ("pluginBehaviorKey" in body) patch["pluginBehaviorKey"] = body["pluginBehaviorKey"] ?? null;
      }
      ...
      botEventLog.record("info", "web", `behavior 已更新 id=${numId} source=${existingRow.source}`, {
        behaviorId: numId,
      });
  ...
  server.post(
    ...
    async (request, reply) => {
      ...
      botEventLog.record("info", "web", `behavior resync id=${numId} result=${result.ok ? "ok" : "fail"}`, {
        behaviorId: numId,
      });
  ...
  server.get(
    "/api/behaviors/audience-summary",
    async (request, reply) => {
      if (!requireBehaviorAdmin(request, reply)) return;

      // 取所有 behavior，前端自行分組
      const rows = await Behavior.findAll({
        order: [["audienceKind", "ASC"], ["audienceUserId", "ASC"], ["audienceGroupName", "ASC"]],
        attributes: ["id", "audienceKind", "audienceUserId", "audienceGroupName", "source", "enabled"],
      });

      const summary = rows.map((r) => ({
        id: r.getDataValue("id") as number,
        audienceKind: r.getDataValue("audienceKind") as string,
        audienceUserId: (r.getDataValue("audienceUserId") as string | null) ?? null,
        audienceGroupName: (r.getDataValue("audienceGroupName") as string | null) ?? null,
